...
import style from "../../Pages/buy/BuyHouseDetail.module.css";

const BuyHouseDetail = () => {
  ...
  const handleClick = () => {
    mutate(`${process.env.REACT_APP_API_URL}/buy/${id}`, {
      method: "POST",
      data: {
        user_id: user.id,
        house_id: id,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await mutate(`${process.env.REACT_APP_API_URL}/buy/${id}/message`, {
        method: "POST",
        data: {
          user_id: user.id,
          house_id: id,
      ...
      {user ? (
        ...
        <div>
          <div className={style.card}>
            ...
            <div className={style.info}>
              <h1 className={style.titleHouse}>HOUSE</h1>
              <p className={style.streetnameHouse}>LOG IN TO SEE THE ADRESS</p>
              <p className={style.houseDescription}>{house.description}</p>
              <p className={style.houseDescription}>€{house.price}</p>
            </div>
          </div>
